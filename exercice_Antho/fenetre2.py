...
class Poste(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("Ajout nouveau employé")
        self.resize(400,500)
        self.centralWidget = QWidget()
        self.setCentralWidget(self.centralWidget)

        self.label1=QLabel("IM employé",self.centralWidget)
        self.text1=QLineEdit(self.centralWidget)

        self.label2=QLabel("Nom et prenom Employé",self.centralWidget)
        ...
        #Bouton ajout nouveau employé
        self.btn1=QPushButton("Ajouter nouvel employé",self.centralWidget)
    ...
    def connection(self,xtuple):
        ...
        try:
            xcur.execute("create table if not exists employe(id_employe integer, nom_employe string(25))")
        except:
            QMessageBox.about(self, "info","Error creation table")

        try:
            xcur.execute("insert into employe(id_employe,nom_employe) values(?,?)",xtuple)
    ...
    def affichage(self):
        ...
        for y1,y2 in enumerate(resultat):
            ...
            for x1,x2 in enumerate(y2):
                ...
                self.table1.setHorizontalHeaderLabels(['ID', 'NOM'])
        xcon.close()
